...
class ExcelWriter:
    ...
    def insert_df2sheet(self, worksheet, data, insert_space, merge_column=None, header=True, index=False, auto_width=False, n_jobs=1):
        """
        ...
        if n_jobs > 1:
            ...
            for i, row in enumerate(dataframe_to_rows(df, header=header, index=index)):
                if i == 0:
                    if header:
                        task = executor.submit(self.insert_rows, worksheet, row, start_row + i, start_col, style="header",
                                         auto_width=auto_width)
                    else:
                        task = executor.submit(self.insert_rows, worksheet, row, start_row + i, start_col, style="first", auto_width=auto_width)
                elif (header and i == len(df)) or (not header and i + 1 == len(df)):
                    task = executor.submit(self.insert_rows, worksheet, row, start_row + i, start_col, style="last", auto_width=auto_width)
                else:
                    task = executor.submit(self.insert_rows, worksheet, row, start_row + i, start_col, auto_width=auto_width,
                                     merge_rows=merge_rows if merge_column else None)
                tasks.append(task)
